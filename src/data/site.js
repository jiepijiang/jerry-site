...
export const timeline = [
  ...
  /**
   * 2026-09-20 Jerry 确认：**嵌入式开发是他当前在做的项目，内容保密。**
   *
   * 所以这里刻意只留一个中性的名字，不写任何细节 —— 别再往下追问、也别
   * 「顺手补全」这段描述。那是他的工作内容，不是这个公开仓库该记录的东西。
   *
   * 日期 2023.1 的来历：它是「自有仓库活动的断点」—— 最后一次 push 是
   * `news_admin` 2023-01-06，之后到 2025-07 才再有动静（中间 2023-09-05
   * 只有一次 fork，不算自己写的项目）。既然这项工作从那时持续到现在，
   * 2023.1 就是它的起点。
   *
   * 复核方式（其余 5 条日期都是这么查出来的）：
   *   gh api "users/jiepijiang/repos?sort=created&per_page=100" \
   *     --jq '.[] | [.name,.created_at[0:10],.pushed_at[0:10],.fork] | @tsv'
   */
  { title: '嵌入式开发（在做）', date: '2023.1' },
 ...
 * 现在换成真实地址；串口助手因为当时找不到稳定可用的在线实现已经移除。
 *
 * 2026-09-20 补记：**找到可用的了** —— AetherPort（`https://serial.xywml.com/`），
 * 纯 Web Serial、协议解析 + YMODEM 升级，作者就是原站站长本人。
 * 它已经加进姊妹项目 jerry-tools 的「开发工具」分类（`seed.js` 的 `b22`）。
 *
 * **这里没有把它做成第 5 张卡，是有意的**：卡片是 `width: calc(25% - 15px)`
 * 的 4 列网格，第 5 张会折到第二行独占一格、页面总高从 1302 涨到约 1416。
 ...
 * 首屏两块 GitHub 相关内容
 * ---------------------------------------------------------------------------
 * ① 贪吃蛇贡献图（`.tanChiShe`）—— 原站那一块，现在用 Jerry 自己的贡献数据生成
 * ② 「最近在做什么」（`.recent`）—— 原站没有，是本项目加的。蛇在上、动态在下
 *
 * 关于 ① 的历史记一笔，免得下次又有人以为它是「挂着别人的数据」：
 *   2026-09-19 那版把它整个删了 —— 原站那张确实是复刻对象 xywml.com **站长的**
 *   贡献图；而换成 Jerry 自己的又几乎全空（371 天里只有 6 天有提交，约 1.6%）。
 *   2026-09-20 Jerry 明确要求把效果做回来：那是他的真实数据，稀疏就稀疏，
 *   比没有强。**别再自作主张删它。**
 *
 * ① 的图由 Platane/snk 生成，`color_snake` 用站点主色 `#bd34fe` ——
 *   Platane 默认的 `purple`（#800080）压在浅灰格子上几乎看不出蛇在哪。
 *   仓库里存一份兜底，构建时 CI 用 `GITHUB_TOKEN` 重新生成覆盖它。
 *
 * ② 的数据也是构建期生成的，不直连 GitHub API：未登录的 api.github.com 限流是
 *   60 次/小时/**出口 IP**，走代理时出口 IP 共享，实测浏览器请求直接吃 403，
 *   那一块就只剩一句「取不到数据」。由 `scripts/gen-recent.mjs` 生成
 *   `public/static/data/recent.json`，CI 里带 token 跑（额度 5000/小时）。
 *
 * 两块都是「来自第三方的数据 → 提交一份 + 构建期刷新」，见 README 的规则。
 */
export const github = {
  username: 'jiepijiang',
  /** ① 贡献贪吃蛇。路径走 asset()，否则 Pages 子路径下 404 */
  snake: asset('static/img/snake.svg'),
