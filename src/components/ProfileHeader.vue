...
/* ---------------------------------------------------------------------------
 * 「最近在做什么」—— 本项目加的一块，跟在贪吃蛇贡献图下面
 * （贪吃蛇原站就有，2026-09-20 放回；两块的来龙去脉见 site.js 的注释）。
 *
...
<template>
  <header>
    ...
    <!-- 贪吃蛇贡献图：原站那一块。2026-09-20 按 Jerry 要求放回来，
         和下面的「最近在做什么」组成首屏的收尾。顺序是蛇在上、动态在下。 -->
    <div class="tanChiShe">
      <img :src="github.snake" alt="GitHub 贡献图，贪吃蛇动画" />
    </div>
...
/* ---------------------------------------------------------------------------
 * 贪吃蛇贡献图 —— 原站那一块，2026-09-20 按 Jerry 要求放回
 *
 * 尺寸照抄原站：`.tanChiShe { width: 85% }`，图本身是 880×192 的比例，
 * 在 880 宽的内容区里量出来是 748×163.2。
 *
 * `img` **故意不加 `display: block`** —— 原站就是行内的，底部那约 3px 的
 * 基线间隙让容器量出来是 166.2，正好对上原站的基准值。加了反而少 3px。
 * ------------------------------------------------------------------------- */
.tanChiShe {
  width: 85%;
}
...
/* ---------------------------------------------------------------------------
 * 「最近在做什么」—— 原站没有这一块，是本项目加的，现在跟在贪吃蛇下面。
 *
 * **容器沿用贪吃蛇的 880/192 宽高比**，所以两块叠起来是 166.2 + 20 + 163.2。
 * ------------------------------------------------------------------------- */
.recent {
