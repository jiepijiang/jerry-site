...
`public/static/img/snake.svg` 由 [Platane/snk](https://github.com/Platane/snk) 生成 ——
把 GitHub 的贡献日历渲染成一条蛇爬过格子的动画 SVG。原站那块是同一个工具做的
（`viewBox` 完全一致），只是数据来自复刻对象站长。

仓库里存了一份兜底，`deploy.yml` 每次构建会重新生成并覆盖它。**同样只能放构建期** ——
贡献日历没有公开的 CORS 接口，浏览器里拉不到。

```bash
# 手动重新生成（要 token，贡献日历走 GraphQL）
GITHUB_TOKEN=$(gh auth token) npx generate-snake-animation@3 \
  --github_user=jiepijiang \
  '--output=public/static/img/snake.svg?color_snake=%23bd34fe'
```

几个刻意的选择：

- **`color_snake` 用站点主色 `#bd34fe`，不是 Platane 默认的 `purple`（#800080）** ——
  默认那个压在浅灰格子上几乎看不出蛇在哪，而「看得出蛇在动」正是这块的全部意义。
- **失败不阻断发布**（`continue-on-error` + 先写 `.snake/` 暂存再覆盖）。
  一张图不该把整站部署搞挂；直接往目标路径写的话，中途失败会留下半截文件。
- **`.tanChiShe img` 故意不加 `display: block`** —— 原站就是行内的，底部那约 3px
  的基线间隙让容器量出来是 **166.2**，正好对上原站基准；加了反而少 3px。

> ⚠️ **这块被删过一次，别再删。** 2026-09-19 那版以「原站用的是复刻对象站长的贡献图，
> 换成自己的又几乎全空（370 格里 359 格没贡献）」为由整体移除了它。
> 2026-09-20 Jerry 明确要求放回来 —— 那是他的真实数据，稀疏就稀疏。
>
> 想让格子密一点，去 GitHub 设置里打开 **Include private contributions on my profile**：
> 它只公开**匿名化的次数**，不泄露仓库名，对保密工作没有影响。
> 打开之后下一次构建就会自动变密。
...
> - **首屏现在是「贪吃蛇 + 最近在做什么」两块叠放**（原站只有贪吃蛇），
>   所以页面总高比原站**高**：1440 宽下 1118 → **1302**。
>   净增 186 ≈ 贪吃蛇容器 166.2 + 两块间距 20。**不是回归，是内容变多了。**
> - 窄屏（≤800px）下「最近在做什么」改成**上下两行**（仓库名一行、`语言 · 时间` 一行），
>   所以 700 / 375 两档各再多 **+24px**。
...
| 页面总高（1440 宽） | 原站 1118px；本站 1302px（本站首屏多一块「最近在做什么」，见上方说明） |
| 响应式 1000×900 / 700×900 / 375×812 | 原站 1327 / 2160 / 1697；本站 1482 / 2328 / 1865（差值见上方说明） |
| 首屏贪吃蛇容器（1440 宽） | **748×166.2 @(395,325)**，与原站逐项一致（含底部 3px 行内基线间隙） |
...
4. **首屏的贪吃蛇贡献图 → 换成了 Jerry 自己的数据**（图由 Platane/snk 生成，见上文）。
   原站那张是同一个工具生成的，但用的是**复刻对象站长的** GitHub 贡献图，
   属于「挂着别人的数据」。
   2026-09-19 那版曾把它整体删掉（理由见上文的 ⚠️ 提示），2026-09-20 按 Jerry 要求放回，
   并在它下面加了「最近在做什么」。**这是本站首屏比原站高 186px 的唯一原因。**
   顺带说明：原站的 `snake-Dark.svg` 本来就是死资源（唯一会换图的 `script.js` 读的是
   那个只写不读的 cookie），所以只保留了一份 `snake.svg`，不再做深浅两套。
...
- **首屏贪吃蛇**：不用手动替换，它读的就是 `github.username` 那个账号的贡献数据，
  每次构建自动刷新。本地想立刻重生成，按上文「贪吃蛇贡献图也是构建期生成的」跑一条命令即可
